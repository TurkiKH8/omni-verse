...
export default function BuyPage() {
  const { t } = useLanguage();
  const plans = [
    { categories: 1, price: 10, label: t.buy.starter,   highlight: false },
    { categories: 3, price: 25, label: t.buy.popular,   highlight: true  },
      ...
      <main className="flex-1 px-6 py-16 relative z-10">
        <div className="max-w-4xl mx-auto flex flex-col gap-10">
          ...
          <div className="text-center">
            <h1 className="text-4xl font-extrabold" style={{ color: "#e8d5a0" }}>{t.buy.title}</h1>
            <p className="mt-3 text-base" style={{ color: "#e8d5a0", opacity: 0.65 }}>{t.buy.subtitle}</p>
          </div>

          {/* Pricing Cards */}
          <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
            {plans.map((plan) => (
              <div key={plan.categories} className="rounded-2xl p-7 flex flex-col items-center gap-4 relative"
                style={{
                ...
                <p className="text-sm text-center" style={{ color: "#e8d5a0", opacity: 0.65 }}>
                  {plan.categories} {plan.categories === 1 ? t.buy.categoryOne : t.buy.categoryMany}
                </p>
                <button className="w-full py-2.5 rounded-full font-bold text-sm mt-2 transition-opacity hover:opacity-90"
                  style={{
          ...
          <div className="rounded-2xl p-8 flex flex-col gap-5 max-w-lg mx-auto w-full" style={{ backgroundColor: "#1e1530", border: "1px solid #2e2050" }}>
            <h2 className="font-bold text-lg" style={{ color: "#e8d5a0" }}>{t.buy.payDetails}</h2>
            <div className="flex flex-col gap-1.5">
              <label className="text-sm font-medium" style={{ color: "#e8d5a0" }}>{t.buy.cardName}</label>
              <input type="text" className="w-full px-4 py-3 rounded-xl text-sm outline-none" style={{ backgroundColor: "#120d1f", border: "1px solid #2e2050", color: "#e8d5a0" }} />
            </div>
            <div className="flex flex-col gap-1.5">
              <label className="text-sm font-medium" style={{ color: "#e8d5a0" }}>{t.buy.cardNumber}</label>
            ...
            <div className="grid grid-cols-2 gap-4">
              <div className="flex flex-col gap-1.5">
                <label className="text-sm font-medium" style={{ color: "#e8d5a0" }}>{t.buy.expiry}</label>
              ...
              <div className="flex flex-col gap-1.5">
                <label className="text-sm font-medium" style={{ color: "#e8d5a0" }}>{t.buy.cvv}</label>
                <input type="text" placeholder="•••" className="w-full px-4 py-3 rounded-xl text-sm outline-none" style={{ backgroundColor: "#120d1f", border: "1px solid #2e2050", color: "#e8d5a0" }} />
              </div>
            </div>
            <p className="text-xs text-center" style={{ color: "#e8d5a0", opacity: 0.4 }}>{t.buy.gatewaySoon}</p>
            <button className="w-full py-3 rounded-full font-bold text-sm transition-opacity hover:opacity-90"
              style={{ backgroundColor: "#d4860a", color: "#120d1f" }} disabled>
              {t.buy.complete}
            </button>
          </div>
        </div>
